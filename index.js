...
function getArtistByIndex(id, name) {
  ...
  console.log(`The artist at index ${name} is ${id[name].name}`)
...
function get20s(arr){
  ...
  var artists20 = []
    ...
    for (x = 0; x < artists.length; x++) {
      ...
      if (artists[x].years.substring(0,4) >= 1800 && artists[x].years.substring(0,4) <= 1900) {
        artists20.push(artists[x]);
    }
  }
  console.log(artists20);
}
...
function checkArtist(str){
  ...
  if (isIn === true) {
    console.log(str + " is in the dataset");
  }

  else {
    console.log(str + " is NOT in the dataset")
  }
}

checkArtist("Anh Truong");
checkArtist("John Doe");
